...
module.exports = {
    ...
    async updateUsers(req, res) {
        try {
            const { id } = req.params
            const { name, email, image, is_admin, telefone } = req.body
            const user = await User.findOne({ where: { id } })
            ...
            if (!user) {
                ...
                const userUpdate = await User.update({ name, email, url: imageUpload.secure_url, image_id: imageUpload.public_id, is_admin, telefone }, { where: { id } })
